...
(async () => {
  ...
  for (const result of results) {
    ...
    assert.equal(result.background, `rgb(${colors[result.theme]})`);
  ...
  const setColor = async color => {
    await page.getByLabel('标题栏颜色', { exact: true }).evaluate((el, value) => {
  ...
  await setColor('#f0e0c0');
  await page.waitForFunction(() => getComputedStyle(document.querySelector('.titlebar')).backgroundColor === 'rgb(240, 224, 192)');
  assert.equal(await page.locator('.titlebar').evaluate(el => getComputedStyle(el).color), 'rgb(20, 34, 49)');
  await page.getByRole('button', { name: '取消', exact: true }).click();
  assert.notEqual(await page.locator('.titlebar').evaluate(el => getComputedStyle(el).backgroundColor), 'rgb(240, 224, 192)');
  await page.getByRole('button', { name: '外观设置', exact: true }).click();
  await setColor('#234567');
  await page.getByRole('button', { name: '完成', exact: true }).click();
  await waitUntil(() => page.evaluate(() => window.cfBridge.getStudyData()), study => study.settings.titlebarColor === '#234567');
  await page.reload();
  await page.waitForFunction(() => getComputedStyle(document.querySelector('.titlebar')).backgroundColor === 'rgb(35, 69, 103)');
  await page.getByRole('button', { name: '外观设置', exact: true }).click();
  await page.getByRole('button', { name: '标题栏跟随主题', exact: true }).click();
  await page.getByRole('button', { name: '完成', exact: true }).click();
  await waitUntil(() => page.evaluate(() => window.cfBridge.getStudyData()), study => study.settings.titlebarColor === '');
  await page.keyboard.press('Tab');
  await maximize.focus();
  const focusOffset = await maximize.evaluate(el => parseFloat(getComputedStyle(el).outlineOffset));
  assert.ok(Math.abs(focusOffset + 3) <= .5, 'focus outline remains inset (allow subpixel rounding at Windows display scaling)');

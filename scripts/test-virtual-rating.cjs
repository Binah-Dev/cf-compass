...
test('two rating views switch user, rank, chart and training without mutating official data',async()=>{
 ...
 const original=JSON.stringify(cache);const view=selectRatingView(cache,study,estimate);
 ...
 assert.equal(resolveTrainingRating(view.user,study).source,'estimated');
 ...
 assert.equal(selectRatingView(cache,study,{...estimate,handle:'foreign'}).ratingMode,'official');
 assert.equal(selectRatingView(cache,study,{...estimate,rating:null}).estimatedPending,true);
});

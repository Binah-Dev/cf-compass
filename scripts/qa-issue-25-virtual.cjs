...
(async () => {
  ...
  await secondSession.locator(".contest-problem-row").first().waitFor();
  ...
  const withoutReference = await page.evaluate(() => window.cfBridge.getContestReplay());
  const referenceEntry = withoutReference.contests.find((entry) => entry.replayId.endsWith(":1700200000"));
  assert.equal(referenceEntry.virtualReference == null, true);
  ...
  steps.push({ name: "virtual-replay-without-estimation" });

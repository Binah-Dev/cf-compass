...
async function main() {
  ...
  try {
    ...
    const originalProblems = contest.problems;
    cache.problems.push(...originalProblems);
    contest.problems = originalProblems.map(({ rating, tags, ...problem }) => problem);
    const recommendation = await service.recommendContestProblems(1900, { review });
    const networkCalls = requestCount;
    const originalLibrary = cache.problems;
    cache.problems = [];
    await assert.rejects(service.recommendContestProblems(1900, { review }), /尚未同步题库/);
    cache.problems = originalLibrary.filter(problem => problem.contestId !== 1900);
    await assert.rejects(service.recommendContestProblems(1900, { review }), /缺少 Rating 或算法标签/);
    assert.equal(requestCount, networkCalls, 'empty candidates must not call the AI provider');
    cache.problems = originalLibrary;
    contest.problems = originalProblems;

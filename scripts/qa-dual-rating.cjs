...
(async()=>{
 ...
 write('combined-rating',{handle:f.cache.handle,records:Object.fromEntries(replay.contests.filter(e=>e.virtualReference).map(e=>[e.replayId,{fingerprint:e.submissionFingerprint,reference:e.virtualReference,checkedAt:Date.now()/1000,startTimeSeconds:e.startTimeSeconds,durationSeconds:e.durationSeconds}]))});
 ...
 // Simulate an old installation's persisted choice; do not touch real user data.
 const legacyStudy=JSON.parse(fs.readFileSync(path.join(profile,'study.json'),'utf8'));
 legacyStudy.trainingProfiles[f.cache.handle.toLowerCase()].displayRatingMode='estimated';write('study',legacyStudy);
 page=await launch();
 assert.equal(await page.getByRole('combobox',{name:'Personal Rating mode',exact:true}).inputValue(),'combined');
 await page.waitForFunction(r=>document.querySelector('.profile-rating .rating-score')?.textContent.replaceAll(',','')===String(r),expected.rating);
